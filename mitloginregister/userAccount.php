...
if(isset($_POST['signupSubmit'])){
    ...
    $redirectURL = ($sessData['status']['type'] == 'success')?'../jdex.php#done':'registration.php';
    ...
    if(!empty($_POST['email'])){
		...
		if($prevUser > 0){
			...
			if($update){
				$resetPassLink = 'http://barisandata.com/mitloginregister/resetPassword.php?fp_code='.$uniqidStr;
				...
				<br/>Baru saja anda menggunakan fitur lupa password untuk melakukan request untuk mengganti password, apabila bukan anda yang request bisa diabaikan.
				<br/>Untuk mereset password, klik link berikut: <a href="'.$resetPassLink.'">'.$resetPassLink.'</a>
				<br/><br/>Regards,
				<br/>Customer Service Mitra Era';
				//set content-type header for sending HTML email
				$headers = "MIME-Version: 1.0" . "\r\n";
				$headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
				//additional headers
				$headers .= 'From: noreply <noreply@barisandata.com>' . "\r\n";
				//send email
				mail($to,$subject,$mailContent,$headers);
				
				$sessData['status']['type'] = 'success';
				$sessData['status']['msg'] = 'Silahkan cek email anda di folder inbox atau folder Spam, kami mengirimkan link reset password.';
			}else{
				$sessData['status']['type'] = 'error';
				$sessData['status']['msg'] = 'Some problem occurred, please try again.';
			}
	...
	//store reset password status into the session
    $_SESSION['sessData'] = $sessData;
    $redirectURL = ($sessData['status']['type'] == 'success')?'../jdex.php#done':'resetPassword.php?fp_code='.$fp_code;
