...
while($jow=mysqli_fetch_array($jiew)){
 ...
 if($jow['status_trans']=='minta')
      { ?>
<script>document.location.href="mobil.php#asrent";</script><?php }
?>
<?php 
...
$input = "SELECT id_users FROM trans_saldo WHERE id_users='$id_users' and statussaldo='minta'";
$result = mysqli_query($mysqli, $input);
$count = mysqli_num_rows($result); // if email not found then register
if($count == 0){
if(mysqli_query($mysqli, "INSERT INTO trans_saldo VALUES('', '$id_users', '$tipesaldo', '$jumlahsaldo', '$statussaldo', '$tgl_request', '$banksaldo', '$namauser', '$nomorrek')"))
		{

			?>
<script>document.location.href="mobil.php#saldo";</script><?php
		}
		else
		{
			?>
