...
$.extend($.jgrid,{
	...
	parseDate : function(format, date, newformat, opts) {
		...
		if(date && date != null) {
			if( !isNaN( date - 0 ) && String(format).toLowerCase() === "u") {
				...
				if (msMatch[3]) {
					...
					timestamp.setTime(Number(Number(timestamp) + (offset * 60 * )));
				}
			} else {
				var offset = 0;
		...
		var 
			...
			N = (w + 6) % 7 + 1,
			z = (new Date(Y, n - 1, j) - new Date(Y, 0, 1)) / 8600,
			flags = {
				// Day
				d: pad(j),
				D: opts.dayNames[w],
	...
	cellWidth : function () {
		var $testDiv = $("<div class='ui-jqgrid' style='left:0px'><table class='ui-jqgrid-btable' style='width:5px;'><tr class='jqgrow'><td style='width:5px;display:block;'></td></tr></table></div>"),
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			...
			pUSD = formatCol( pos,irow, v, srvr, rowId, rdata);
			return "<td role=\"gridcell\" "+pUSD ">"+v+"</td>";
		},
		addMulti = function(rowid,pos,irow,checked){
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/>",
			pUSD = formatCol( pos,irow,'',null, rowid, true);
			return "<td role=\"gridcell\" "+pUSD ">"+v+"</td>";
		},
		addRowNum = function (pos,irow,pG,rN) {
			var v =  (parseInt(pG,10)-1)*parseInt(rN,10)+1+irow,
			pUSD = formatCol( pos,irow,v, null, irow, true);
			return "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+pUSD ">"+v+"</td>";
		},
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			if (!gxml) { gxml = []; }
			var gl = gxml.length, j=0, gUSD ata=[], rn = parseInt(ts.p.rowNum,10), br=ts.p.scroll?$.jgrid.randId():1, altr;
			...
			if(ts.p.gridview === true) {
				...
				if(ts.p.grouping) {
					if(!locdata) {
						self.jqGrid('groupingRender',gUSD ata,ts.p.colModel.length, ts.p.page, rn);
					}
					gUSD ata = null;
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			var afterInsRow = $.isFunction(ts.p.afterInsertRow), gUSD ata=[],hiderow=false, groupingPrepare;
			if(ts.p.grouping)  {
				hiderow = ts.p.groupingView.groupCollapse === true;
				groupingPrepare = $.jgrid.getMethod("groupingPrepare");
			}
		...
		addLocalData = function() {
			...
			var gUSD iew = ts.p.grouping ? ts.p.groupingView : false, lengUSD  gin;
			$.each(ts.p.colModel,function(){
				...
				if(ts.p.grouping ) {
					for(gin =0, lengUSD = gUSD iew.groupField.length; gin< lengUSD  gin++) {
						if( this.name === gUSD iew.groupField[gin]) {
			...
			if(ts.p.grouping) {
				for(gin=0; gin<lengUSD gin++) {
					query.orderBy(grindexes[gin],gUSD iew.groupOrder[gin],grtypes[gin].stype, grtypes[gin].srcfmt);
				}
			}
			if(ts.p.multiSort) {
				$.each(st,function(i){
			...
			recordspeUSD ge = parseInt(ts.p.rowNum,10),
			total = queryResults.length,
			page = parseInt(ts.p.page,10),
			totalpages = Math.ceil(total / recordspeUSD ge),
			retresult = {};
			if((ts.p.search || ts.p.resetsearch) && ts.p.grouping && ts.p.groupingView._locgr) {
				ts.p.groupingView.groups =[];
				var j, gUSD epare = $.jgrid.getMethod("groupingPrepare"), key, udc;
				...
				for(j=0; j<total; j++) {
					...
					gUSD epare.call($(ts),queryResults[j],j, recordspeUSD ge );
				}
			}
			queryResults = queryResults.slice( (page-1)*recordspeUSD ge , page*recordspeUSD ge );
			query = null;
			cmtypes = null;
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				if(ts.p.grouping) {
					...
					var gUSD = ts.p.groupingView, gi, gs="";
					for(gi=0;gi<gUSD groupField.length;gi++) {
						var index = gUSD groupField[gi];
						$.each(ts.p.colModel, function(cmIndex, cmValue) {
							if (cmValue.name === index && cmValue.index){
								index = cmValue.index;
							}
						} );
						gs += index +" "+gUSD groupOrder[gi]+", ";
					}
		...
		setPager = function (pgid, tp){
			...
			$("td#"+pgid+"_"+ts.p.pageUSD s,"#"+pgcnt).append(pgl);
			...
			if(twd > 0) {
				...
				$("td#"+pgid+"_"+ts.p.pageUSD s,"#"+pgcnt).width(twd);
			}
			ts.p._nvtd = [];
...
$.jgrid.extend({
	...
	setSelection : function(selection,onsr, e) {
		return this.each(function(){
			...
			function scrGrid(iR){
				...
				USD s = $($t.rows[iR]).position().top,
				rh = $t.rows[iR].clientHeight;
				if(USD s+rh >= ch+st) { $($t.grid.bDiv)[0].scrollTop = USD s-(ch+st)+rh+st; }
				else if(USD s < ch+st) {
					if(USD s < st) {
						$($t.grid.bDiv)[0].scrollTop = USD s;
					}
				}
			}
	...
	addRowData : function(rowid,rdata,pos,src) {
		if(!pos) {pos = "last";}
		var success = false, nm, row, gi, si, ni,sind, i, v, pUSD "", aradd, cnm, cn, data, cm, id;
		if(rdata) {
			if($.isArray(rdata)) {
				aradd=true;
				pos = "last";
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(ni){
						pUSD = t.formatCol(0,1,'',null,rowid, true);
						row[row.length] = "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+pUSD ">0</td>";
					}
					if(gi) {
						v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+t.p.id+"_"+rowid+"\" class=\"cbox\"/>";
						pUSD = t.formatCol(ni,1,'', null, rowid, true);
						row[row.length] = "<td role=\"gridcell\" "+pUSD ">"+v+"</td>";
					}
					if(si) {
					...
					for(i = gi+si+ni; i < t.p.colModel.length;i++){
						...
						pUSD = t.formatCol(i,1,v, data, rowid, lcdata);
						row[row.length] = "<td role=\"gridcell\" "+pUSD ">"+v+"</td>";
					}
	...
	setLabel : function(colname, nData, prop, attUSD ){
		return this.each(function(){
			var $t = this, pos=-1;
			if(!$t.grid) {return;}
			...
			if(pos>=0) {
				...
				if(typeof attUSD === 'object') {$(thecol).attr(attUSD ;}
			}
		});
	},
	setCell : function(rowid,colname,nData,cssp,attUSD  forceupd) {
		return this.each(function(){
			...
			if(pos>=0) {
				...
				if (ind){
					...
					if(typeof attUSD === 'object') {$(tcell).attr(attUSD ;}
				}
			}
		});
	},
...
$.jqm = {
hash:{},
open:function(s,t){var h=H[s],c=h.c,cc='.'+c.closeClass,z=(parseInt(h.w.css('z-index')));z=(z>0)?z:;var o=$('<div></div>').css({height:'100%',width:'100%',position:'fixed',left:0,top:0,'z-index':z-1,opacity:c.overlay/100});if(h.a)return F;h.t=t;h.a=true;h.w.css('z-index',z);
...
$.fn.jqFilter = function( arg ) {
	...
	return this.each( function() {
		...
		*/
		...
		/* moving to common
		randId = function() {
			return Math.floor(Math.random()*0).toString();
		};
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.bind('change',function() {
				...
				$(".selectopts",tUSD r).empty().append( s );
				$(".selectopts",tUSD r)[0].selectedIndex = 0;
				if( $.jgrid.msie && $.jgrid.msiever() < 9) {
					var sw = parseInt($("select.selectopts",tUSD r)[0].offsetWidth, 10) + 1;
					$(".selectopts",tUSD r).width( sw );
					$(".selectopts",tUSD r).css("width","auto");
				}
				// data
				$(".data",tUSD r).empty().append( elm );
				$.jgrid.bindEv.call($t, elm, cm.searchoptions);
				$(".input-elm",tUSD r).bind('change',function( e ) {
					var elem = e.target;
...
$.jgrid.extend({
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			onBeforeShow = $.isFunction(USD ge[$t.p.id].beforeShowForm) ? USD ge[$t.p.id].beforeShowForm : false,
			onAfterShow = $.isFunction(USD ge[$t.p.id].afterShowForm) ? USD ge[$t.p.id].afterShowForm : false,
			onBeforeInit = $.isFunction(USD ge[$t.p.id].beforeInitData) ? USD ge[$t.p.id].beforeInitData : false,
			onInitializeForm = $.isFunction(USD ge[$t.p.id].onInitializeForm) ? USD ge[$t.p.id].onInitializeForm : false,
			showFrm = true,
			...
			if (rowid === "new") {
				rowid = "_empty";
				frmoper = "add";
				p.caption=USD ge[$t.p.id].addCaption;
			} else {
				p.caption=USD ge[$t.p.id].editCaption;
				frmoper = "edit";
			}
			if(!p.recreateForm) {
				if( $($t).data("formProp") ) {
					$.extend(USD ge[$(this)[0].p.id], $($t).data("formProp"));
				}
			}
			var closeovrl = true;
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						USD = parseInt(frmopt.rowpos,10) || cnt+1,
						...
						if(USD ge[$t.p.id].checkOnSubmit || USD ge[$t.p.id].checkOnUpdate) {USD ge[$t.p.id]._savedData[nm] = tmp;}
						$(elc).addClass("FormElement");
						if( $.inArray(this.edittype, ['text','textarea','password','select']) > -1) {
							$(elc).addClass("ui-widget-content ui-corner-all");
						}
						trdata = $(tb).find("tr[rowpos="+USD "]");
						if(frmopt.rowabove) {
						...
						if ( trdata.length===0 ) {
							trdata = $("<tr "+dc+" rowpos='"+USD "'></tr>").addClass("FormData").attr("id","tr_"+nm);
							$(trdata).append(tmpl);
							$(tb).append(trdata);
							trdata[0].USD = USD 
						}
				...
				if( cnt > 0) {
					...
					idrow[0].USD = cnt+999;
					$(tb).append(idrow);
					if(USD ge[$t.p.id].checkOnSubmit || USD ge[$t.p.id].checkOnUpdate) {USD ge[$t.p.id]._savedData[obj.p.id+"_id"] = rowid;}
				}
				return retpos;
			}
			function fillData(rowid,obj,fmid){
				var nm,cnt=0,tmp, fld,opt,vl,vlc;
				if(USD ge[$t.p.id].checkOnSubmit || USD ge[$t.p.id].checkOnUpdate) {USD ge[$t.p.id]._savedData = {};USD ge[$t.p.id]._savedData[obj.p.id+"_id"]=rowid;}
				...
				if(rowid === '_empty') {
					$(cm).each(function(){
						...
						if(fld && fld.length && fld[0] !== null) {
							...
							if(USD ge[$t.p.id].checkOnSubmit===true || USD ge[$t.p.id].checkOnUpdate) {USD ge[$t.p.id]._savedData[nm] = vl;}
						}
					});
					$("#id_g","#"+fmid).val(rowid);
					return;
				...
				$('td[role="gridcell"]',tre).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && cm[i].editable===true) {
						...
						if($t.p.autoencode) {tmp = $.jgrid.htmlDecode(tmp);}
						if(USD ge[$t.p.id].checkOnSubmit===true || USD ge[$t.p.id].checkOnUpdate) {USD ge[$t.p.id]._savedData[nm] = tmp;}
						nm = $.jgrid.jqID(nm);
			...
			function postIt() {
				...
				if($.isFunction(USD ge[$t.p.id].beforeCheckValues)) {
					retvals = USD ge[$t.p.id].beforeCheckValues.call($t, postdata,$("#"+frmgr),frmoper);
					if(retvals && typeof retvals === 'object') {postdata = retvals;}
				}
				for( key in postdata ){
				...
				setNulls();
				if(ret[0]) {
					onCS = $($t).triggerHandler("jqGridAddEditClickSubmit", [USD ge[$t.p.id], postdata, frmoper]);
					if( onCS === undefined && $.isFunction( USD ge[$t.p.id].onclickSubmit)) { 
						onCS = USD ge[$t.p.id].onclickSubmit.call($t, USD ge[$t.p.id], postdata, frmoper) || {}; 
					}
					ret = $($t).triggerHandler("jqGridAddEditBeforeSubmit", [postdata, $("#"+frmgr), frmoper]);
					if(ret === undefined) {
						ret = [true,"",""];
					}
					if( ret[0] && $.isFunction(USD ge[$t.p.id].beforeSubmit))  {
						ret = USD ge[$t.p.id].beforeSubmit.call($t,postdata,$("#"+frmgr), frmoper);
					}
				}

				if(ret[0] && !USD ge[$t.p.id].processing) {
					USD ge[$t.p.id].processing = true;
					...
					postdata = $.extend(postdata,USD ge[$t.p.id].editData,onCS);
					...
					var ajaxOptions = $.extend({
						url: USD ge[$t.p.id].url || $($t).jqGrid('getGridParam','editurl'),
						type: USD ge[$t.p.id].mtype,
						data: $.isFunction(USD ge[$t.p.id].serializeEditData) ? USD ge[$t.p.id].serializeEditData.call($t,postdata) :  postdata,
						complete:function(data,status){
							...
							if(data.status >= 300 && data.status !== 304) {
								...
								if ($.isFunction(USD ge[$t.p.id].errorTextFormat)) {
									ret[1] = USD ge[$t.p.id].errorTextFormat.call($t, data, frmoper);
								} else {
								...
								if( ret[0] && $.isFunction(USD ge[$t.p.id].afterSubmit) ) {
									ret = USD ge[$t.p.id].afterSubmit.call($t, data,postdata, frmoper);
								}
							}
							if(ret[0] === false) {
								...
								//USD ge[$t.p.id].reloadAfterSubmit = USD ge[$t.p.id].reloadAfterSubmit && $t.p.datatype != "local";
								// the action is add
								if(postdata[oper] === opers.addoper ) {
									//id processing
									...
									if(USD ge[$t.p.id].reloadAfterSubmit) {
									...
									if(USD ge[$t.p.id].closeAfterAdd) {
										if($t.p.treeGrid !== true){
											$($t).jqGrid("setSelection",ret[2]);
										}
										$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal,onClose: USD ge[$t.p.id].onClose});
									} else if (USD ge[$t.p.id].clearAfterAdd) {
									...
									if(USD ge[$t.p.id].reloadAfterSubmit) {
										$($t).trigger("reloadGrid");
										if( !USD ge[$t.p.id].closeAfterEdit ) {setTimeout(function(){$($t).jqGrid("setSelection",postdata[idname]);},);}
									} else {
										if($t.p.treeGrid === true) {
									...
									if(USD ge[$t.p.id].closeAfterEdit) {$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal,onClose: USD ge[$t.p.id].onClose});}
								}
								if($.isFunction(USD ge[$t.p.id].afterComplete)) {
									copydata = data;
									setTimeout(function(){
										$($t).triggerHandler("jqGridAddEditAfterComplete", [copydata, postdata, $("#"+frmgr), frmoper]);
										USD ge[$t.p.id].afterComplete.call($t, copydata, postdata, $("#"+frmgr), frmoper);
										copydata=null;
									},500);
								}
								if(USD ge[$t.p.id].checkOnSubmit || USD ge[$t.p.id].checkOnUpdate) {
									$("#"+frmgr).data("disabled",false);
									if(USD ge[$t.p.id]._savedData[$t.p.id+"_id"] !== "_empty"){
										for(key in USD ge[$t.p.id]._savedData) {
											if(USD ge[$t.p.id]._savedData.hasOwnProperty(key) && postdata[key]) {
												USD ge[$t.p.id]._savedData[key] = postdata[key];
											}
										}
									}
								}
							}
							USD ge[$t.p.id].processing=false;
							$("#sData", frmtb+"_2").removeClass('ui-state-active');
							try{$(':input:visible',"#"+frmgr)[0].focus();} catch (e){}
						}
					}, $.jgrid.ajaxOptions, USD ge[$t.p.id].ajaxEditOptions );

					if (!ajaxOptions.url && !USD ge[$t.p.id].useDataProxy) {
						if ($.isFunction($t.p.dataProxy)) {
							USD ge[$t.p.id].useDataProxy = true;
						} else {
							ret[0]=false;ret[1] += " "+$.jgrid.errors.nourl;
						}
					}
					if (ret[0]) {
						if (USD ge[$t.p.id].useDataProxy) {
							...
							if(dpret[0] === false ) {
								...
								if(ajaxOptions.data.oper === opers.addoper && USD ge[$t.p.id].closeAfterAdd ) {
									$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: USD ge[$t.p.id].onClose});
								}
								if(ajaxOptions.data.oper === opers.editoper && USD ge[$t.p.id].closeAfterEdit ) {
									$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: USD ge[$t.p.id].onClose});
								}
							}
						} else {
			...
			function checkUpdates () {
				...
				if(USD ge[$t.p.id].checkOnUpdate) {
					postdata = {};
					getFormData();
					diff = compareData(postdata,USD ge[$t.p.id]._savedData);
					if(diff) {
			...
			var dh = isNaN(USD ge[$(this)[0].p.id].dataheight) ? USD ge[$(this)[0].p.id].dataheight : USD ge[$(this)[0].p.id].dataheight+"px",
			dw = isNaN(USD ge[$(this)[0].p.id].datawidth) ? USD ge[$(this)[0].p.id].datawidth : USD ge[$(this)[0].p.id].datawidth+"px",
			...
			$(tbl).append(flr);
			//topinfo
			flr = $("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"+(maxCols*2)+"'>"+USD ge[$t.p.id].topinfo+"</td></tr>");
			flr[0].USD = 0;
			$(tbl).append(flr);
			// set the id.
			...
			bt += "<tr style='display:none' class='binfo'><td class='bottominfo' colspan='2'>"+USD ge[$t.p.id].bottominfo+"</td></tr>";
			bt += "</tbody></table>";
			if(maxRows >  0) {
				var sd=[];
			...
			$.jgrid.createModal(IDs,tms, USD ge[$(this)[0].p.id] ,"#gview_"+$.jgrid.jqID($t.p.id),$("#gbox_"+$.jgrid.jqID($t.p.id))[0]);
			if(rtlb) {
				$("#pData, #nData",frmtb+"_2").css("float","right");
				$(".EditButton",frmtb+"_2").css("text-align","left");
			}
			if(USD ge[$t.p.id].topinfo) {$(".tinfo",frmtb).show();}
			if(USD ge[$t.p.id].bottominfo) {$(".binfo",frmtb+"_2").show();}
			tms = null;bt=null;
			$("#"+$.jgrid.jqID(IDs.themodal)).keydown( function( e ) {
				var wkey = e.target;
				if ($("#"+frmgr).data("disabled")===true ) {return false;}//??
				if(USD ge[$t.p.id].savekey[0] === true && e.which === USD ge[$t.p.id].savekey[1]) { // save
				...
				if(e.which === 27) {
					...
					if(cle)	{$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:p.gbox,jqm:p.jqModal, onClose: USD ge[$t.p.id].onClose});}
					return false;
				}
				if(USD ge[$t.p.id].navkeys[0]===true) {
					if($("#id_g",frmtb).val() === "_empty") {return true;}
					if(e.which === USD ge[$t.p.id].navkeys[1]){ //up
						$("#pData", frmtb+"_2").trigger("click");
						return false;
					}
					if(e.which === USD ge[$t.p.id].navkeys[2]){ //down
			...
			if(rowid==="_empty" || !USD ge[$t.p.id].viewPagerButtons) {$("#pData,#nData",frmtb+"_2").hide();} else {$("#pData,#nData",frmtb+"_2").show();}
			$($t).triggerHandler("jqGridAddEditBeforeShowForm", [$("#"+frmgr), frmoper]);
			if(onBeforeShow) { onBeforeShow.call($t, $("#"+frmgr), frmoper);}
			$("#"+$.jgrid.jqID(IDs.themodal)).data("onClose",USD ge[$t.p.id].onClose);
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if(!p.recreateForm) {
				if( $($t).data("viewProp") ) {
					$.extend(USD ge[$(this)[0].p.id], $($t).data("viewProp"));
				}
			}
			function focusaref(){ //Sfari 3 issues
				if(USD ge[$t.p.id].closeOnEscape===true || USD ge[$t.p.id].navkeys[0]===true) {
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && viewfld) {
						...
						trdata = $(tb).find("tr[rowpos="+USD "]");
						if ( trdata.length===0 ) {
							trdata = $("<tr "+dc+" rowpos='"+USD "'></tr>").addClass("FormData").attr("id","trv_"+nm);
							$(trdata).append(tmpl);
							$(tb).append(trdata);
							trdata[0].USD = USD 
						}
			...
			$("#"+IDs.themodal).keydown( function( e ) {
				if(e.which === 27) {
					if(USD ge[$t.p.id].closeOnEscape) {$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:p.gbox,jqm:p.jqModal, onClose: p.onClose});}
					return false;
				}
				if(p.navkeys[0]===true) {
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if (!$t.grid ) {return;}
			if(!rowids) {return;}
			var onBeforeShow = $.isFunction( USD ge[$t.p.id].beforeShowForm  ),
			onAfterShow = $.isFunction( USD ge[$t.p.id].afterShowForm ),
			onBeforeInit = $.isFunction(USD ge[$t.p.id].beforeInitData) ? USD ge[$t.p.id].beforeInitData : false,
			gID = $t.p.id, onCS = {},
			showFrm = true,
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#DelError","#"+dtbl).hide();
				if( USD ge[$t.p.id].processing === true) {
					USD ge[$t.p.id].processing=false;
					$("#dData", "#"+dtbl).removeClass('ui-state-active');
				}
				if(onBeforeShow) {USD ge[$t.p.id].beforeShowForm.call($t,$("#"+dtbl));}
				$.jgrid.viewModal("#"+$.jgrid.jqID(IDs.themodal),{gbox:"#gbox_"+$.jgrid.jqID(gID),jqm:USD ge[$t.p.id].jqModal,jqM: false, overlay: USD ge[$t.p.id].overlay, modal:USD ge[$t.p.id].modal});
				if(onAfterShow) {USD ge[$t.p.id].afterShowForm.call($t,$("#"+dtbl));}
			} else {
				var dh = isNaN(USD ge[$t.p.id].dataheight) ? USD ge[$t.p.id].dataheight : USD ge[$t.p.id].dataheight+"px",
				...
				tbl += "<tr><td class=\"delmsg\" style=\"white-space:pre;\">"+USD ge[$t.p.id].msg+"</td></tr><tr><td >&#160;</td></tr>";
				// buttons at footer
				tbl += "</tbody></table></div>";
				...
				p.delicon = $.extend([true,"left","ui-icon-scissors"],USD ge[$t.p.id].delicon);
				p.cancelicon = $.extend([true,"left","ui-icon-cancel"],USD ge[$t.p.id].cancelicon);
				if(p.delicon[0]===true) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					onCS = {};
					if( $.isFunction( USD ge[$t.p.id].onclickSubmit ) ) {onCS = USD ge[$t.p.id].onclickSubmit.call($t,USD ge[$t.p.id], postdata) || {};}
					if( $.isFunction( USD ge[$t.p.id].beforeSubmit ) ) {ret = USD ge[$t.p.id].beforeSubmit.call($t,postdata);}
					if(ret[0] && !USD ge[$t.p.id].processing) {
						USD ge[$t.p.id].processing = true;
						opers = $t.p.prmNames;
						postd = $.extend({},USD ge[$t.p.id].delData, onCS);
						oper = opers.oper;
						...
						var ajaxOptions = $.extend({
							url: USD ge[$t.p.id].url || $($t).jqGrid('getGridParam','editurl'),
							type: USD ge[$t.p.id].mtype,
							data: $.isFunction(USD ge[$t.p.id].serializeDelData) ? USD ge[$t.p.id].serializeDelData.call($t,postd) : postd,
							complete:function(data,status){
								var i;
								if(data.status >= 300 && data.status !== 304) {
									ret[0] = false;
									if ($.isFunction(USD ge[$t.p.id].errorTextFormat)) {
										ret[1] = USD ge[$t.p.id].errorTextFormat.call($t,data);
									} else {
										ret[1] = status + " Status: '" + data.statusText + "'. Error code: " + data.status;
									}
								} else {
									// data is posted successful
									// execute aftersubmit with the returned data from server
									if( $.isFunction( USD ge[$t.p.id].afterSubmit ) ) {
										ret = USD ge[$t.p.id].afterSubmit.call($t,data,postd);
									}
								}
								if(ret[0] === false) {
									...
									if(USD ge[$t.p.id].reloadAfterSubmit && $t.p.datatype !== "local") {
										$($t).trigger("reloadGrid");
									} else {
										if($t.p.treeGrid===true){
									...
									if($.isFunction(USD ge[$t.p.id].afterComplete)) {
										setTimeout(function(){USD ge[$t.p.id].afterComplete.call($t,data,postdata);},500);
									}
								}
								USD ge[$t.p.id].processing=false;
								$("#dData", "#"+dtbl+"_2").removeClass('ui-state-active');
								if(ret[0]) {$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: USD ge[$t.p.id].onClose});}
							}
						}, $.jgrid.ajaxOptions, USD ge[$t.p.id].ajaxDelOptions);
				...
				$("#eData", "#"+dtbl+"_2").click(function(){
					$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:USD ge[$t.p.id].jqModal, onClose: USD ge[$t.p.id].onClose});
					return false;
				});
				if(onBeforeShow) {USD ge[$t.p.id].beforeShowForm.call($t,$("#"+dtbl));}
				$.jgrid.viewModal("#"+$.jgrid.jqID(IDs.themodal),{gbox:"#gbox_"+$.jgrid.jqID(gID),jqm:USD ge[$t.p.id].jqModal, overlay: USD ge[$t.p.id].overlay, modal:USD ge[$t.p.id].modal});
				if(onAfterShow) {USD ge[$t.p.id].afterShowForm.call($t,$("#"+dtbl));}
			}
			if(USD ge[$t.p.id].closeOnEscape===true) {
				setTimeout(function(){$(".ui-jqdialog-titlebar-close","#"+$.jgrid.jqID(IDs.modalhead)).focus();},0);
			}
		});
	},
...
addSubGridCell :function (pos,iRow) {
	var pUSD '',ic,sid;
	this.each(function(){
		pUSD = this.formatCol(pos,iRow);
		sid= this.p.id;
		ic = this.p.subGridOptions.plusicon;
	});
	return "<td role=\"gridcell\" aria-describedby=\""+sid+"_subgrid\" class=\"ui-sgcollapsed sgcollapsed\" "+pUSD "><a style='cursor:pointer;'><span class='ui-icon "+ic+"'></span></a></td>";
},
...
$.jgrid.extend({
	...
	setTreeGrid : function() {
		return this.each(function (){
			...
			$t.p.gridview =  true;
			if($t.p.rowTotal === null ) { $t.p.rowNum = 0; }
...
$.jgrid.extend({
	...
	groupingRender : function (grdata, colspans, page, rn ) {
		return this.each(function(){
			...
			var sumreverse = $.makeArray(gUSD groupSummary);
			sumreverse.reverse();
			$.each(gUSD groups,function(i,n){
				...
				if(gUSD groupSummaryPos[n.idx] === 'header')  {
					str += "<tr id=\""+hid+"\"" +(gUSD groupCollapse && n.idx>0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\">"+icon+$.jgrid.template(gUSD groupText[n.idx], gv, n.cnt, n.summary)+"</td>";
					str += buildSummaryTd(i, n.idx-1, gUSD groups, 1);
					str += "</tr>";
				} else {
					str += "<tr id=\""+hid+"\"" +(gUSD groupCollapse && n.idx>0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\" colspan=\""+colspans+"\">"+icon+$.jgrid.template(gUSD groupText[n.idx], gv, n.cnt, n.summary)+"</td></tr>";
				}
				var leaf = len-1 === n.idx; 
				if( leaf ) {
					var gg = gUSD groups[i+1], kk, ik, offset = 0, sgr = n.startRow,
					end = gg !== undefined ?  gUSD groups[i+1].startRow : grdata.length;
					if(gUSD _locgr) {
						offset = (page-1)*rn;
				...
				return $.extend({
					...
					drop: function(ev, ui) {
						...
						if(ui.helper.dropped) {
							...
							// NULL is inteUSD eted as undefined while null as object
