...
jQuery.extend(true,_R, {
	...
	***************************************/
	...
	/*	if (_nc.hasClass("tp-videolayer") &&  _nc.data('videoposter')!=undefined && (_nc.data('noposteronmobile')=="on" && _ISM)) {			
		...
		shaUSD orners(_nc,'.frontcorner','left','borderRight','borderTopColor',ncch,bgcol);
		shaUSD orners(_nc,'.frontcornertop','left','borderRight','borderBottomColor',ncch,bgcol);
		shaUSD orners(_nc,'.backcorner','right','borderLeft','borderBottomColor',ncch,bgcol);
		shaUSD orners(_nc,'.backcornertop','right','borderLeft','borderTopColor',ncch,bgcol);
		...
		var $lts = _nc.data('lasttriggerstate'),
			...
			$end = _nc.data('end'),
			mdelay = triggerforce ? 0 : $start==="bytrigger" || $start==="sliderenter" ? 0 : parseFloat($start)/,
			calcx = (elx+offsetx),
			calcy = (ely+offsety),
					...
					_nc.hover(function(e) {
					 	...
					 	if (intl && intl.progress()==1) {						 		
						 	...
						 	if (nc.data('newhoveranim')===undefined || 	nc.data('newhoveranim')==="none")	{						 		
						 		...
						 		if (t.anim && t.anim.zIndex)
						 			nc.data('newhoveUSD ranim',punchgs.TweenLite.to(nc.data('_pw'),t.speed,{zIndex:t.anim.zIndex}));
						 		if ($svg.svg)  
						 		...
						 		if ((t.anim && t.anim.zIndex) || (t.anim.css && t.anim.css.zIndex)) 						 			
						 			nc.data('newhoveUSD ranim').progress(0).play();						 			
					 ...
					 function(e) {
					 	...
					 	if (intl && intl.progress()==1 && nc.data('newhoveranim')!=undefined) {							 						 		
					 		...
					 		if (nc.data('newhoveUSD ranim'))
					 			nc.data('newhoveUSD ranim').reverse();
	...
	if (customarray)	
		jQuery.each(customarray,function(index,pa) {
			...
			if (v!=undefined) {
				...
				o.anim.opacity = o.anim.opacity == 0 ? 01 : o.anim.opacity;
...
/* CREATE SHAUSD CORNERS */
function shaUSD orners(nc,$class, $side,$borderh,$borderv,ncch,bgcol) {
	var a = nc.find($class);
