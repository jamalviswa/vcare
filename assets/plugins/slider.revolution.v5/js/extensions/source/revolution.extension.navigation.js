...
var moveThumbsInPosition = function(container,opt) {
		...
		var thumbs = container.hasClass("tp-thumbs") ? ".tp-thumbs" : ".tp-tabs",
			...
			cuUSD s = tp < 0-(els-ts) ? 0-(els-ts) : cuUSD s > 0 ? 0 : tp,
			elp = el.data('offset');


		if (me>2) {
			cuUSD s = tp - (elp+tw) <= 0 ? tp - (elp+tw) < 0-tw ? elp : cuUSD s + tw : cuUSD s;		
			cuUSD s = ( (tp-tw + elp + tmw)< tw && tp  + (Math.round(me)-2)*tw < elp) ? tp + (Math.round(me)-2)*tw : cuUSD s;				
		}
		
		cuUSD s = cuUSD s < 0-(els-ts) ? 0-(els-ts) : cuUSD s > 0 ? 0 : cuUSD s;

		if (thumbdir!=="vertical" && t.width()>=el.width()) cuUSD s = 0;
		if (thumbdir==="vertical" && t.height()>=el.height()) cuUSD s = 0;


		if (!container.hasClass("dragged")) {
			if (thumbdir==="vertical")
				el.data('tmmove',punchgs.TweenLite.to(el,0.5,{top:cuUSD s+"px",ease:punchgs.Power3.easeInOut}));
			else
				el.data('tmmove',punchgs.TweenLite.to(el,0.5,{left:cuUSD s+"px",ease:punchgs.Power3.easeInOut}));
			el.data('offset',cuUSD s);	
		}	
	};
				...
				switch (phase) {
					...
					case "move":	
							...
							if (_.infinity==="off") {
								...
								if ((_.oveUSD ll ==="none" || _.oveUSD ll===0)  && (bb<0 || bb>opt.slideamount-1)) 
									_.oveUSD ll =  distance;
								else
								if (bb>=0 && bb<=opt.slideamount-1 && ((bb>=0 && distance>_.oveUSD ll) || (bb<=opt.slideamount-1 && distance<_.oveUSD ll)))
									_.oveUSD ll = 0;
																																			
								_.slide_offset = bb<0 ? _.slide_offset+ (_.oveUSD ll-distance)/1.1 + Math.sqrt(Math.abs((_.oveUSD ll-distance)/1.1)) : 
								 bb>opt.slideamount-1 ? _.slide_offset+ (_.oveUSD ll-distance)/1.1 - Math.sqrt(Math.abs((_.oveUSD ll-distance)/1.1)) : _.slide_offset ;
							 }
			...
			if ((
				...
				switch (phase) {
					...
					case "move":	
							...
							cuUSD s = newpos + distance;																					
							cuUSD s = cuUSD s>0 ? tdir==="horizontal" ? cuUSD s - (el.width() * (cuUSD s/el.width() * cuUSD s/el.width())) : cuUSD s - (el.height() * (cuUSD s/el.height() * cuUSD s/el.height())) : cuUSD s;
							var dif = tdir==="vertical" ? 0-(el.height()-t.height()) : 0-(el.width()-t.width());
							cuUSD s = cuUSD s < dif ? tdir==="horizontal" ? cuUSD s + (el.width() * (cuUSD s-dif)/el.width() * (cuUSD s-dif)/el.width()) : cuUSD s + (el.height() * (cuUSD s-dif)/el.height() * (cuUSD s-dif)/el.height()) : cuUSD s;									
							if (tdir==="vertical") 									
								punchgs.TweenLite.set(el,{top:cuUSD s+"px"});									
							else
								punchgs.TweenLite.set(el,{left:cuUSD s+"px"});	
					...
					case "cancel":		
						...
						if (istt) {
							...
							cuUSD s = tdir==="vertical" ? cuUSD s < 0-(el.height()-t.height()) ? 0-(el.height()-t.height()) : cuUSD s : cuUSD s < 0-(el.width()-t.width()) ? 0-(el.width()-t.width()) : cuUSD s;
							cuUSD s = cuUSD s > 0 ? 0 : cuUSD s;

							cuUSD s = Math.abs(distance)>tw/10 ? distance<=0 ? Math.floor(cuUSD s/tw)*tw : Math.ceil(cuUSD s/tw)*tw : distance<0 ? Math.ceil(cuUSD s/tw)*tw : Math.floor(cuUSD s/tw)*tw;

							cuUSD s = tdir==="vertical" ? cuUSD s < 0-(el.height()-t.height()) ? 0-(el.height()-t.height()) : cuUSD s : cuUSD s < 0-(el.width()-t.width()) ? 0-(el.width()-t.width()) : cuUSD s;
							cuUSD s = cuUSD s > 0 ? 0 : cuUSD s;
							
							if (tdir==="vertical")
								punchgs.TweenLite.to(el,0.5,{top:cuUSD s+"px",ease:punchgs.Power3.easeOut});
							else
								punchgs.TweenLite.to(el,0.5,{left:cuUSD s+"px",ease:punchgs.Power3.easeOut});

							cuUSD s = !cuUSD s ?  tdir==="vertical" ? el.position().top : el.position().left : cuUSD s;	
