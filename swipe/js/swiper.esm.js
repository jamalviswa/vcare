...
function onTouchEnd (event) {
  ...
  if (params.freeMode) {
    ...
    if (params.freeModeMomentum) {
      ...
      let momentumDuration =  * params.freeModeMomentumRatio;
...
class Swiper extends SwiperClass {
  constructor(...args) {
    ...
    const swipeUSD rams = Utils.extend({}, defaults);
...
const Observer = {
  ...
  init() {
    ...
    if (swiper.params.observeParents) {
      const containeUSD rents = swiper.$el.parents();
      for (let i = 0; i < containeUSD rents.length; i += 1) {
...
const Lazy = {
  ...
  load() {
    const swiper = this;
    const {
      $wrapperEl, params: swipeUSD rams, slides, activeIndex,
    } = swiper;
    const isVirtual = swiper.virtual && swipeUSD rams.virtual.enabled;
    const params = swipeUSD rams.lazy;

    let slidesPerView = swipeUSD rams.slidesPerView;
    ...
    function slideExist(index$$1) {
      if (isVirtual) {
        if ($wrapperEl.children(`.${swipeUSD rams.slideClass}[data-swiper-slide-index="${index$$1}"]`).length) {
          return true;
        }
    ...
    if (swiper.params.watchSlidesVisibility) {
      $wrapperEl.children(`.${swipeUSD rams.slideVisibleClass}`).each((elIndex, slideEl) => {
    ...
    if (params.loadPrevNext) {
      if (slidesPerView > 1 || (params.loadPrevNextAmount && params.loadPrevNextAmount > 1)) {
        ...
        const nextSlide = $wrapperEl.children(`.${swipeUSD rams.slideNextClass}`);
        if (nextSlide.length > 0) swiper.lazy.loadInSlide(slideIndex(nextSlide));

        const prevSlide = $wrapperEl.children(`.${swipeUSD rams.slidePrevClass}`);
