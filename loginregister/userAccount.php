...
if(isset($_POST['signupSubmit'])){
    ...
    if(!empty($_POST['email'])){
		...
		if($prevUser > 0){
			...
			if($update){
				$resetPassLink = 'http://barisandata.com/loginregister/resetPassword.php?fp_code='.$uniqidStr;
				...
				<br/>Hi, You request change password in app Barisandata, Ignore when not right.
				<br/>For Reset your password, Please Follow this link: <a href="'.$resetPassLink.'">'.$resetPassLink.'</a>
				<br/><br/>Regards,
				<br/>Administrators';
				...
				//additional headers
				$headers .= 'From: noreply Administrator noreply<admin@barisandata.com>' . "\r\n";
				//send email
				mail($to,$subject,$mailContent,$headers);
				
				$sessData['status']['type'] = 'success';
				$sessData['status']['msg'] = 'Please check your email inbox or spam folder, We Send link to reset Your password.';
			}else{
				$sessData['status']['type'] = 'error';
				$sessData['status']['msg'] = 'Some problem occurred, please try again.';
			}
	...
	//store reset password status into the session
    $_SESSION['sessData'] = $sessData;
    $redirectURL = ($sessData['status']['type'] == 'success')?'../done.php#done':'resetPassword.php?fp_code='.$fp_code;
