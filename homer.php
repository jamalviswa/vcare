...
<link rel="stylesheet"href="themes/base/jquery.ui.all.css"><script src="http://maps.google.com/maps/api/js?key=AIzaSyBg32t8VhZ2AcEy8IpIzBK9PtcOCVQF1v0"></script>
...
<div style="color:#fff;margin-top:20px;font-size:15px;font-weight:bold;font-family:Segoe UI light;float:right;padding-right:20px;">Destination Adress (Drop Off)
</div></div></div>
...
if (empty($_POST['lat'])) {
   ?>
<script>window.alert("Please Drag location point");window.history.back(-2);</script><?php
  return false;
}
?>

<script>function showlocation(){if("geolocation" in navigator){navigator.geolocation.getCurrentPosition(callback,error)}else{console.warn("geolocation IS NOT available")}}function error(a){console.warn("ERROR("+a.code+"): "+a.message)}function initialize(){var c=[];var a=new google.maps.Map(document.getElementById("map"),{mapTypeId:google.maps.MapTypeId.ROADMAP});function h(j){var l=j.coords.latitude;var k=j.coords.longitude;document.getElementById("lat").value=l;document.getElementById("lng").value=k}var d=new google.maps.LatLng(-7.0274846,108.2405507);var e=new google.maps.Marker({position:d,draggable:true,map:a,zoom:18,animation:google.maps.Animation.DROP});c.push(e);var b=new google.maps.LatLngBounds(new google.maps.LatLng(-7.0274846,108.2405507));a.fitBounds(b);var g=document.getElementById("pac-input");a.controls[google.maps.ControlPosition.TOP_CENTER].push(g);var i=new google.maps.places.SearchBox(g);google.maps.event.addListener(i,"places_changed",function(){var l=i.getPlaces();if(l.length==0){return}for(var m=0,k;k=c[m];m++){k.setMap(null)}c=[];var n=new google.maps.LatLngBounds();for(var m=0,j;j=l[m];m++){var o={url:j.icon,size:new google.maps.Size(90,90),origin:new google.maps.Point(0,0),anchor:new google.maps.Point(17,34),scaledSize:new google.maps.Size(25,25)};var k=new google.maps.Marker({draggable:true,map:a,icon:o,title:j.name,zoom:14,animation:google.maps.Animation.DROP,position:j.geometry.location});google.maps.event.addListener(k,"dragend",function(p){f(this.getPosition())});google.maps.event.addListener(k,"click",function(p){f(this.getPosition())});c.push(k);n.extend(j.geometry.location)}a.fitBounds(n)});google.maps.event.addListener(a,"bounds_changed",function(){var j=a.getBounds();i.setBounds(j)});function f(j){document.getElementById("lat").value=j.lat();document.getElementById("lng").value=j.lng()}}initialize();</script>
<input type="hidden" name="id_users"type="text" value="<?php echo $_SESSION['user']?>"/>
<input type="hidden" name="asal"value="<?php echo $_POST["lat"]; ?>,<?php echo $_POST["lng"]; ?>"/>
<input type="hidden" name="latfrom"value="<?php echo $_POST["lat"]; ?>"/>
<input type="hidden" name="lngfrom"value="<?php echo $_POST["lng"]; ?>"/>
...
<center><small>Click next to view price</small></center></td>
