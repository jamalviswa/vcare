...
$r=$_POST['r'];
$query=mysqli_query($mysqli, "select * from users where sebagai='user' and first_name like '%".$r."%' or noktp like '%".$r."%' or email like '%".$r."%' LIMIT 400");
	...
	while ($data=mysqli_fetch_array($query)) // perulangna untuk menampilkan data
	{
		...
		echo "<table width='100%' style='color:#444;border-bottom:1px solid #d4d4d4;font-size:14px;padding:4px;'>
				<tr>
    				...
    				<td style='padding:10px;border-left:1px solid grey'width='20%'><a target=_blank href=lihat.php?id=".$data['id'].">show</a></td>
				</tr>
			</table>";	
	}
}
