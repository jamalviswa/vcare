...
$result = mysqli_query($mysqli, "SELECT * FROM transaksi WHERE YEAR(tanggal) = YEAR(NOW( )) order by tanggal ASC");
?>
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0"/> 
<link rel="stylesheet"type="text/css"href="../button.css"/>
<html>
<body style="padding:25px;"onload="window.print()"><center>
<h3>Data Transaksi Trol Pada Tahun <?php echo date('Y'); ?></h3></center>
<div id="content">
...
<b> USD  <?php
$query = "SELECT sum(harga) AS total FROM transaksi where status_trans='otw' or status_trans='finish' and YEAR(tanggal) = YEAR(NOW( ))"; 
...
<b> USD  <?php
$query = "SELECT sum(harga) AS total FROM transaksi where YEAR(tanggal) = YEAR(NOW( ))"; 
$result = mysqli_query($mysqli, $query); 
$values = mysqli_fetch_assoc($result); 
$num_rows = $values['total']; 
$laba = number_format($num_rows,0,",",".");
...
<br />
<div class="container" style="background-color:#fff;"><br><br><center><h4 style="color: #6f6f6f;">Summary transaction</h4>
			<!-- Codrops top bar -->
		<div class="section group">
<br>
	<div class="col span_2_of_2">

	Profit on <?php echo date('Y'); ?>
<br>
	<?php
include_once '../dbconnect.php';	
...
$id = $_SESSION['user'];
$banjal = mysqli_query($mysqli,"SELECT tanggal FROM transaksi WHERE YEAR(tanggal) = YEAR(NOW( )) order by tanggal ASC");
$jemhasilan = mysqli_query($mysqli,"SELECT * FROM transaksi WHERE YEAR(tanggal) = YEAR(NOW( )) order by tanggal ASC");
?>
        <style type="text/css">
            .fonte {
