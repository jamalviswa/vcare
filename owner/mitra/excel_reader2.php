...
define('SMALL_BLOCK_THRESHOLD', 0x);
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
						...
						$xf['borderBottom'] = $this->lineStyles[($border & 0x) >> 12];
						
						$xf['borderLeftColor'] = ($border & 0x70) >> 16;
						$xf['borderRightColor'] = ($border & 0x3F00) >> 23;
	...
	function _parsesheet($spos) {
			...
			switch ($code) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_COLINFO:
					...
					for ($coli = $colfrom; $coli <= $colto; $coli++) {
						$this->colInfo[$this->sn][$coli+1] = Array('width' => $cw, 'xf' => $cxf, 'hidden' => ($co & 0x01), 'collapsed' => ($co & 0x) >> 12);
					}
					break;
		...
		function _getCellDetails($spos,$numValue,$column) {
			...
			if ($type == 'date') {
				...
				$raw = $numValue;
				$fractionalDay = $numValue - floor($numValue) + 0001; // The 0001 is to fix for php/excel fractional diffs
	...
	function createNumber($spos) {
		...
		$sign = ($rknumhigh & 0x) >> 31;
		$exp =  ($rknumhigh & 0x7f00) >> 20;
		$mantissa = (0x00 | ($rknumhigh & 0fffff));
		$mantissalow1 = ($rknumlow & 0x) >> 31;
