...
<div style="border-bottom:1px solid #8c8c8c;color:#616161;font-size:14px;font-weight:bold">Lihat Lokasi Partner Seluruhnya
</div>
<center>
...
function addMarker(lat, lng, info) {
            ...
            bounds.extend(pt);
var iconBase = 'https://sinyonya.com/beko/owner/marker.png';
          ...
          <?php		  
            $query = mysqli_query($mysqli, "SELECT * FROM mitra WHERE latmitra!='' AND latmitra IS NOT NULL");
          while ($data = mysqli_fetch_array($query)) {
            $lat = $data['latmitra'];
            $lng = $data['lngmitra'];
			$nama_mitra = $data['nama_mitra'];
			$mitra_email = $data['mitra_email'];
			$nomorhp = $data['nomorhp'];
			$no_ktp = $data['no_ktp'];
			$sebagai = $data['sebagai'];
			$idunik = $data['idunik'];
            echo ("addMarker($lat, $lng, '<a style=font-size:12px;color:>$nama_mitra<br>($sebagai)<br><br>No. KTP: $no_ktp<br>Phone: $nomorhp<br></a>');\n");                        
          }
          ?>
        }
