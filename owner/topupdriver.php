...
?>
<?php
$jiew=mysqli_query($mysqli, "SELECT * FROM trans_sopir where idsopir=".$_SESSION['user']);
...
<?php
$jiew=mysqli_query($mysqli, "SELECT * FROM transaksi where id_mitra=".$_SESSION['user']);
while($jow=mysqli_fetch_array($jiew)){
	?>
<?php 
 if($jow['status_trans']=='dijemput')
      { ?>
<script>document.location.href="jemput.php#row";</script><?php }
?>
<?php
if($jow['status_trans']=='otw')
      {?>
<script>document.location.href="jemput.php#otw";</script><?php }};?>
<?php
...
$count = mysqli_num_rows($result); // if email not found then register
if($count == 0){
if(mysqli_query($mysqli, "INSERT INTO `trans_sopir` (`idsaldo`, `idsopir`, `tipesaldo`, `jumlahsaldo`, `statussaldo`, `tgl_request`, `banksaldo`, `namauser`, `nomorrek`) VALUES (NULL, '$idsopir', 'topup', '$jumlahsaldo', 'minta', '$tgl_request', '$banksaldo', '$namauser', '$nomorrek');"))
		{
			?>
<script>document.location.href="saldohabis.php";</script><?php
		}
		else
		{
			?>
...
<div style="margin-top:20px;font-size:18px;font-family:Segoe UI light"><center>Deposit</center>
</div>
</div>
...
2. Transfer to administrator bank account
</center></td>
...
<center style="padding:10px;background-color:rgba(1, 102, 185, 0.76);border-bottom:5px solid #5cb55c">folowing this step for topup,-</center>
